...
pro fiss_pca_conv_old, file, outfile,  ncoeff=ncoeff, pfile=pfile, init=init, wr=wr , eval=eval ;, crcor=crcor
;+
;   NAME: FISS_PCA_CONV
...
nw=fxpar(h,'NAXIS1')
ny=fxpar(h,'NAXIS2')
if nd eq 3 then nx=fxpar(h,'NAXIS3') else nx=1
...
if n_elements(ncoeff) eq 0 then ncoeff=30
;evec[*, ncoeff]=0.
