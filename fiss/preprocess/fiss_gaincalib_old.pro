function fiss_gaincalib_old, logsp, x, dx, object=object,  maxiter=maxiter, $
...
;-   2018 May, Minor debugging (array in alignoffset function)
...
if dx lt 0 then begin
 sh =  alignoffset(total((logsp[*, *, order[k+1]]-Flat)[0:(nx-1+dx), ny/2-10:ny/2+10], 2)/21.#replicate(1.,3), $
                   total((logsp[*, *, order[k]]-Flat)[-dx:(nx-1), ny/2-10:ny/2+10], 2)/21.#replicate(1.,3), cor)
endif else begin
 sh =  alignoffset(total((logsp[*, *, order[k+1]]-Flat)[dx:(nx-1),ny/2-10:ny/2+10], 2)/21.#replicate(1.,3), $
                   total((logsp[*, *, order[k]]-Flat)[0:(nx-1-dx), ny/2-10:ny/2+10], 2)/21.#replicate(1.,3), cor)
endelse
