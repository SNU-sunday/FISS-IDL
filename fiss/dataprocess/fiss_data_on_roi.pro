pro fiss_data_on_roi, align, k, xr, yr, ds, roi, outfile=outfile, alignfile=alignfile
...
data[*, s[j]]=data[*, s[j]]*((x ge 0)*((x+2) le Nx)*(yy[s[j]] ge 0)*((yy[s[j]]+2) lt Ny))
endfor
endif
endfor
...
roi={data:data, k:k, wv:wv, xg:xg, yg:yg}
if n_elements(outfile) eq 1 then save, filename=outfile+'_roi.sav', roi
